...
a = read_invent_ch4("2015", "domcom")
b = read_invent_ch4("2015", "energyprod")
c = read_invent_ch4("2015", "offshore")
d = read_invent_ch4("2015", "othertrans")
e = read_invent_ch4("2015", "roadtrans")
f = read_invent_ch4("2015", "total")
    
fract = a.flux + b.flux + c.flux + d.flux + e.flux / f.flux
test = fract.reindex_like(Intem2012, method='nearest', tolerance=0.01)

tenerg12 = test * intem_by_year(0)
te12 = tenerg12.sum()
    
te12 = te12 * 16 * 31536000 * 28 / 10000000000000
te12 = te12.to_pandas()
...
for name, in zip(years):
    ...
    a = read_invent_ch4_old(name, "domcom")
    b = read_invent_ch4_old(name, "energyprod")
    c = read_invent_ch4_old(name, "offshore")
    d = read_invent_ch4_old(name, "othertrans")
    e = read_invent_ch4_old(name, "roadtrans")
    f = read_invent_ch4_old(name, "total")
...
for name, in zip(years):
    ...
    a = read_invent_ch4_old(name, "agric")
    b = read_invent_ch4_old(name, "total")
...
for name, in zip(years):
    ...
    if name == "2012":
        tLULUCF12 = test * intem_by_year(0)
        tLU12 = tLULUCF12.sum()
    if name == "2013":
        tLULUCF13 = test * intem_by_year(1)
        tLU13 = tLULUCF13.sum()
    if name == "2014":
        tLULUCF14 = test * intem_by_year(2)
        tLU14 = tLULUCF14.sum()
    if name == "2015":
        tLULUCF15 = test * intem_by_year(3)
        tLU15 = tLULUCF15.sum()
    
tLULUCF = xr.concat((tLU12, tLU13, tLU14, tLU15), dim="time")
tLULUCF = tLULUCF * 16 * 31536000 * 28 / 1000000000000
tLULUCF = tLULUCF.to_pandas()
...
for date in year:
    ...
    ch4_gt_all.loc[date, "1. Energy_std"] = ch4_fract.loc[date, "InTEM_std"] * (fe.loc[date]/100)
    ch4_gt_all.loc[date, "2.  Industrial processes_std"] = ch4_fract.loc[date, "InTEM_std"] * (find.loc[date]/100)
    ch4_gt_all.loc[date, "3.  Agriculture_std"] = ch4_fract.loc[date, "InTEM_std"] * (fa.loc[date]/100)
    #ch4_gt_all.loc[date, "4. Land use, land-use change and forestry"] = tLULUCF.loc[date]
    ch4_gt_all.loc[date, "5.  Waste_std"] = ch4_fract.loc[date, "InTEM_std"] * (fw.loc[date]/100)
